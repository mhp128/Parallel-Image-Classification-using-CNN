                ...
                for col in range(output_width):
                    for fillterIdx in range(n_filters):
                        ...
                        out_grad_val = output_gradient[i_batch,
                                                       fillterIdx, row, col]
                        filter_gradient[fillterIdx] += self.inputs[i_batch, :,
                                                                   row_start:row_end, col_start:col_end] * out_grad_val
                        input_gradient[i_batch, :, row_start:row_end,
        ...
        for c in range(num_channels):
            for h in range(output_height):
                for w in range(output_width):
                    ...
                    outputs[:, c, h, w] = np.max(
                        inputs[:, c, h_start:h_end, w_start:w_end], axis=(1, 2))
        return outputs
